...
class WarlockSelectChoicesSkillProficiencies:
    def __init__(self, level, json_path=None):
        self.level = level
        if json_path is None:
            self.json_path = os.path.join(basedir, 'app', 'db', 'character_classes', 'warlock.json')
        else:
    ...
    def get_level_options(self):
        ...
        return {
            'level': self.level,
            'class_name': data.get('name', 'Warlock'),
